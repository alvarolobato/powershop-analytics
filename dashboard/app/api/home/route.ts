    ...
    const [
      ...
      query(
        `SELECT t.codigo,
         ...
         FROM ps_tiendas t
         LEFT JOIN (
           SELECT tienda, COALESCE(SUM(total_si) FILTER (WHERE entrada), 0) - COALESCE(SUM(total_si) FILTER (WHERE NOT entrada), 0) AS sales, COUNT(DISTINCT reg_ventas)::int AS tickets
           FROM ps_ventas
