...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      // Retail ops prev day: tickets, gross, devoluciones for previous day
      // Uses same-hour cutoff so comparison is fair on live days.
      query(
        `SELECT
      ...
      // Retail ops prev month margin: previous full calendar month
      query(
        `SELECT
           COALESCE(SUM(lv.total_si), 0)::numeric AS rev,
           COALESCE(SUM(lv.total_coste_si), 0)::numeric AS cost
    ...
    const opsRetail: Metric[] = [
      ...
      },
      {
        ...
        value: margenPct,
        format: "pct",
        delta: prevMargenPct !== 0 ? safeRatio(margenPct, prevMargenPct) : null,
        sub: "vs mes ant",
      },
      {
        id: "devolu",
