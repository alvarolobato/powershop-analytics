 ...
 * Date filters use :curr_from/:curr_to tokens substituted at render time from the DateRangePicker.
 * YoY KPI uses :curr_from/:curr_to for the current period and subtracts 1 year for the prior period.
 */
...
WHERE "entrada" = true
  AND "tienda" <> '99'
  AND "fecha_creacion" >= :curr_from AND "fecha_creacion" <= :curr_to`,
          format: "currency",
          prefix: "€",
        },
        {
...
WHERE "abono" = false
  AND "fecha_factura" >= :curr_from AND "fecha_factura" <= :curr_to`,
          format: "currency",
          prefix: "€",
        },
        {
...
(
  ...
  FROM "public"."ps_ventas"
  WHERE "entrada" = true AND "tienda" <> '99'
    AND "fecha_creacion" >= :curr_from - INTERVAL '1 year'
    AND "fecha_creacion" <= :curr_to - INTERVAL '1 year'
) prev`,
          format: "percent",
        },
      ],
