...
export function DashboardRenderer({ spec, refreshKey = 0, dateRange }: DashboardRendererProps) {
  ...
  useEffect(() => {
    ...
    // dateRange is intentionally NOT in the dep array here — the date range
    // is applied at the page level by incrementing refreshKey when it changes.
    // The SQL queries in the spec are expected to use dynamic date expressions
    // or be regenerated by the LLM with the selected date range baked in.
