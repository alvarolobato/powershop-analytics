...
### D-030: Watchdog cron planned bump to `*/30` + event triggers (YAML pending human commit) to compensate for GitHub schedule queue saturation — 2026-05-11
...
### 2026-05-09
- WrenAI knowledge corpus unified with dashboard source MDs (issue #524, closes #509): `scripts/wren-push-metadata.py` rewritten as a Markdown renderer. Hard-coded `INSTRUCTIONS` (46 entries) and `SQL_PAIRS` (52 entries) Python arrays removed. The script now parses `## LLM:rules` (JSON instruction arrays) and `## LLM:sql-pairs` (`### question + ```sql``` blocks`) from the same `SOURCE_MDS` list as `dashboard/scripts/build-knowledge.ts`. New parser functions: `parse_marker_sections`, `extract_instructions`, `extract_sql_pairs`, `transform_date_placeholders`, `load_knowledge_from_mds`. Date placeholders (`:curr_from`, `:curr_to`, `:comp_from`, `:comp_to`) are transformed to native PostgreSQL `CURRENT_DATE` / `DATE_TRUNC` expressions before insertion into WrenAI's SQLite, keeping source MDs format-neutral. Added `--dry-run` flag (prints counts + JSON sample without network calls). New counts after unification: **47 instructions** (3 previously Python-only added to `docs/etl-sync-strategy.md ## LLM:rules`: date-filter guardrail, COUNT(DISTINCT) JOIN rule, GUARDIA DE MAGNITUD), **56 SQL pairs** (from `docs/dashboard/sql-pairs.md`). Tests: `scripts/tests/test_wren_push_metadata.py` (26 unit tests). `AGENTS.md` "Knowledge file maintenance" section updated: both `npm run build:knowledge` (dashboard) and `ps wren push` (WrenAI) are required when updating any source MD.

### 2026-05-07
- Stock dashboard: size-breakdown widgets (issue #488, sub-task of #480). Added 3 new widgets to `dashboard/lib/templates/stock.ts` after `stock-por-familia`: (1) `stock-distribucion-tallas` — bar chart of units per size with all global filters; (2) `stock-roturas-por-talla` — table of (Familia, Talla) combos with CASE aggregates for Con Stock / Sin Stock / Total Refs / % Rotura, HAVING to filter rows with no stockout; (3) `stock-articulos-por-talla` — top-50 accumulator table (Familia | Talla | Referencia | Descripción | Stock sorted DESC). Known limitation: `stock-roturas-por-talla` only detects stockouts for size/family combos that have appeared in `ps_stock_tienda`; sizes never stocked are invisible. 3 SQL pairs added to `knowledge.ts`. Test file `dashboard/lib/templates/__tests__/stock.test.ts` added (10 widget assertions).
