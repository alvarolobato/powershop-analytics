...
- **Suspected p4d row-decode desync — unconfirmed, flagged not fixed (D-050)**: On 2026-08-28 a `LineasVentas` fetch produced one row with garbage values (`Mes = -1801453568`, `PrecioNetoSI` decoding to `NaN`) immediately followed by ~60 entirely-NULL rows, all within the same `safe_fetch()` result. This pattern — one half-decoded row, then a run of NULL rows until the next fetch/batch boundary — is consistent with a buffer/row-boundary desync in the `p4d` driver or `etl/db/fourd.py::safe_fetch()`'s row decoding, not with genuinely NULL source data (LineasVentas.RegLineas is the PK and should never be NULL in 4D). Not investigated further — needs live 4D access and a byte-level look at the driver's row-boundary handling to confirm. `etl/db/postgres.py::upsert()` now pre-filters NULL/NaN-PK rows so this no longer takes a whole batch down (see D-050), but the root cause of *why* the decode produces these rows in the first place is still open.
