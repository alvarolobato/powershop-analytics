...
- **Exportaciones `Stock1..Stock34` are 16-bit integers in 4D** (`_USER_COLUMNS`: `DATA_TYPE = 3`, `DATA_LENGTH = 2` for all 34 columns). Through **4D SQL / p4d**, negative per-size quantities often appear as **unsigned** values (`65535` = `−1`, `65534` = `−2`). The **`CCStock`** field on the same row (= `Exportaciones.CCStock`) is **Real** (`DATA_TYPE = 6`) and matches the **signed** total the POS shows. The ETL decodes slot stock with `decode_signed_int16_word()` in `etl/db/fourd.py` before writing `ps_stock_tienda.stock`.
- **`CCStock` table (central warehouse) `Stock1..Stock34` are also 16-bit integers** (confirmed via `_USER_COLUMNS` 2026-05-01: `DATA_TYPE=3, DATA_LENGTH=2`). Despite the name similarity with `Exportaciones.CCStock` (Real), the CCStock *table* column `Stock` (singular, Real type 6) is different from `Stock1..Stock34` (int16). The ETL applies `decode_signed_int16_word()` to `CCStock.Stock1..Stock34` just as it does for Exportaciones. Live sample data confirms unsigned overflow values (65535) in CCStock.Stock1.
...
- **Suspected p4d row-decode desync — flagged, guarded, not driver-fixed (D-050 / D-051)**: On 2026-08-28 a `LineasVentas` fetch produced one row with garbage values (`Mes = -1801453568`, `PrecioNetoSI` decoding to `NaN`) immediately followed by ~60 entirely-NULL rows, all within the same `safe_fetch()` result. This pattern — one half-decoded row, then a run of NULL rows until the next fetch/batch boundary — is consistent with a buffer/row-boundary desync in the `p4d` driver, not with genuinely NULL source data (LineasVentas.RegLineas is the PK and should never be NULL in 4D). `etl/db/postgres.py::upsert()` pre-filters NULL/NaN-PK rows so a corrupted batch no longer takes good rows down with it (D-050), and `etl/db/fourd.py::safe_fetch()` now scans every fetch for this shape and re-executes once to discriminate transient corruption from real data before it ever reaches `upsert()` (D-051).
  - **p4d pages results 100 rows at a time** (`p4d.Cursor.pagesize = 100`, verified in the installed driver at `p4d/p4d.py`; passed to `fourd_exec_statement(query, pagesize)`), re-fetching pages over the socket mid-resultset. A transient network hiccup during a mid-result page fetch plausibly desyncs the C parser *within that page* — one garbage row, then NULLs until the page boundary resyncs it. Falsifiable prediction: an anomalous run should be confined to one 100-row-aligned window, ending at an index ≡ 99 (mod 100). `safe_fetch()`'s evidence records `page_aligned_end` / `run_start_mod_100` / `run_end_mod_100` so real incidents can be checked against this.
  - **A genuine upstream driver bug, found while confirming the above (not fixed — do not patch/vendor p4d)**: `p4d.Cursor.fetchone()`'s `VK_REAL`/`VK_FLOAT` branch is missing an `else`:
    ```python
