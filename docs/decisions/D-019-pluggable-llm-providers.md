...
- **Telemetry**: `llm_usage` and `llm_tool_calls` gain `llm_provider` + `llm_driver`. *(Superseded by [D-043](D-043-cli-usage-metering-and-budget.md): CLI rows originally logged zero `estimated_cost_usd` and the daily budget summed OpenRouter-estimated spend only — both now use the CLI's real reported usage/cost across every call site, and the budget applies to every provider.)*
