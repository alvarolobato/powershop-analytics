...
**Decision**: `FREE_CHAT_TOOLS` expone las 11 herramientas de inspección + `start_dashboard_generation` + `set_title` (13 total). Eran 10 hasta que `search_knowledge` se sumó al conjunto de inspección: el modelo sólo veía 12 de 113 documentos, así que 163 consultas SQL validadas le resultaban invisibles. `set_title` es la única herramienta de escritura permitida en free-chat: escribe únicamente el campo `title` de la conversación actual, usando `AND title IS NULL` para garantizar idempotencia. Las herramientas de escritura de dashboards (`apply_dashboard_modification`, `submit_dashboard_analysis`) están registradas en `FULL_DASHBOARD_TOOLS` (catálogo completo, para uso futuro) pero **no incluidas** en `FREE_CHAT_TOOLS`. Cuando el usuario pide un dashboard nuevo desde el chat libre, el LLM llama `start_dashboard_generation`, que crea el dashboard y ejecuta `POST /api/conversations/:id/handoff-to-dashboard` para mutar la conversación a `mode='modify'`. El usuario recibe un enlace en el chat (`/dashboard/:newId?continue=:convId`) y al navegar allí `ChatSidebar` carga la conversación preexistente en la pestaña Modificar, con todo el historial del free-chat visible.
