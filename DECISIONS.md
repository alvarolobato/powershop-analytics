...
| [D-032](docs/decisions/D-032-free-chat-tools.md) | Free-chat (planned, #616) uses `FREE_CHAT_TOOLS` (10 inspection + `start_dashboard_generation`). Write tools live only in `FULL_DASHBOARD_TOOLS`. Handoff via `POST /api/conversations/:id/handoff-to-dashboard`. |
